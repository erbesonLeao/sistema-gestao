// frontend/src/components/Layout.js - VERSÃO FINAL SEGURA
...
import { 
  ...
  Avatar, Menu, MenuItem, IconButton, Tooltip, Badge, CircularProgress
} from '@mui/material';
...
function Layout() {
  ...
  // AQUI ESTÁ A PROTEÇÃO: Se o 'user' ainda não foi carregado,
  // mostramos um loading em vez de tentar renderizar e quebrar.
  if (!user) {
    return (
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      <AppBar position="fixed" sx={{ zIndex: (theme) => theme.zIndex.drawer + 1 }}>
        <Toolbar>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            {/* Como já verificamos o 'user' acima, este 'user &&' é uma segurança extra */}
            {user && (
              <div>
      ...
      {/* ... O resto do seu código (Menu de Notificação, Drawer, etc.) continua aqui ... */}
