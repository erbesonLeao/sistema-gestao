// frontend/src/pages/FuncionariosPage.js - VERSÃO CORRIGIDA E OTIMIZADA
...
function FuncionariosPage() {
    ...
    const buscarFuncionarios = useCallback(async () => {
        ...
        try {
            ...
            const resposta = await api.get('/api/funcionarios/');
    ...
    const handleSalvar = async () => {
        if (modoEdicao) {
            ...
            try {
                ...
                await api.put(`/api/funcionarios/${idFuncionarioEdit}/`, payload);
                handleFecharDialog();
                buscarFuncionarios();
            } catch (erro) {
    ...
    const handleEditarClick = (funcionario) => {
        ...
        setFormState({
            ...
            user: { username: '', password: '', password2: '' } // Campos de usuário ficam vazios na edição
        });
        setDialogAberto(true);
    };
    ...
    const handleConfirmarApagar = async () => {
        try {
            ...
            await api.delete(`/api/funcionarios/${idFuncionarioApagar}/`);
    ...
    return (
        <Container maxWidth="lg">
            ...
            {/* Diálogo para Criar/Editar Funcionário */}
            <Dialog open={dialogAberto} onClose={handleFecharDialog}>
                ...
                <DialogContent>
                    ...
                    {/* Campos de login aparecem apenas no modo de criação */}
                    {!modoEdicao && (<>
            ...
            {/* Diálogo para Apagar Funcionário */}
